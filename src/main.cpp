...
#if defined(NDEBUG)
# error "Cum cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xebfccc0bfc297d7d1b40605f223b4e8cdb56b9147e4b686489404a72441ebcfe");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cum: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Cum Signed Message:\n";
...
{
    // Cum: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 20160 * 60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cum: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cum: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1509323716, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "Cum";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 45 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d3363f6d6c96cac1cbbf7230469a9ffcf38c76c86e45fede34d67b1d3a4fd2e20a0401ad564524ec9a5bae6398a04ec2cdc241e3865589927fa0ab046dc81c0e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1509323716;
        ...
        assert(block.hashMerkleRoot == uint256("0xfe26f3f71184b5f6b7f8707e1eba6b0195bdae238be9b1c11fce28c5b48f70dc"));
...
unsigned char pchMessageStart[4] = { 0x31, 0xd3, 0x3f, 0xfb }; // Cum: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CumMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CumMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CumMiner(CWallet *pwallet)
{
    printf("CumMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cum-miner");
    ...
    try { loop {
        ...
        printf("Running CumMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CumMiner, pwallet));
}
